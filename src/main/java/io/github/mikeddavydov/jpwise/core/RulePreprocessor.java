...
/**
 * Preprocesses test input by identifying and adding missing compatibility
 * rules.
 * Rules are identified by analyzing existing rules and their interactions with
 * other parameters.
 * The preprocessor returns a new TestInput instance with augmented rules.
 */
public class RulePreprocessor {
    private static final Logger logger = LoggerFactory.getLogger(RulePreprocessor.class);

    /**
     * Preprocesses the test input by identifying missing rules and returns a new
     * TestInput
     * instance containing new TestParameter objects with augmented compatibility
     * rules.
     * The original TestInput and its TestParameters are not modified.
     *
    ...
    public TestInput preprocess(TestInput originalInput) {
        ...
        if (rulesToAdd.isEmpty()) {
            ...
            for (TestParameter p : originalInput.getTestParameters()) {
                ...
                processedInput.add(new TestParameter(p.getName(), new ArrayList<>(p.getPartitions()),
        ...
        for (RuleAddition addition : rulesToAdd) {
            newRulesForTargetParameters
                    ...
                    .add(addition.rule);
            logger.debug("Rule from {} identified to be added to {}", addition.sourceParam.getName(),
                    addition.targetParam.getName());
        }

        for (TestParameter originalParam : originalInput.getTestParameters()) {
            List<CompatibilityPredicate> augmentedDependencies = new ArrayList<>(originalParam.getDependencies());
            List<CompatibilityPredicate> additionalRulesForThisParam = newRulesForTargetParameters
                    .get(originalParam.getName());
            if (additionalRulesForThisParam != null) {
                for (CompatibilityPredicate newRule : additionalRulesForThisParam) {
                    if (!augmentedDependencies.contains(newRule)) {
                        ...
                        logger.debug("Adding new rule to {}. Total rules now: {}", originalParam.getName(),
    ...
    private static class RuleAddition {
        ...
        RuleAddition(CompatibilityPredicate rule, TestParameter sourceParam, TestParameter targetParam) {
            this.rule = rule;
    ...
    private List<RuleAddition> findMissingRulesForParameter(
            TestParameter sourceParam,
            List<TestParameter> allParameters) {
        List<RuleAddition> rulesToAdd = new ArrayList<>();
    ...
    private boolean shouldAddRule(
            CompatibilityPredicate rule,
            TestParameter sourceParam,
            TestParameter targetParam) {
    ...
    /**
     * Tests if a rule interacts with a target parameter by checking if any
     * combination
     * of values between the source and target parameters would be incompatible.
     */
    private boolean ruleInteractsWithParameter(
        ...
        for (EquivalencePartition sv : sourceParam.getPartitions()) {
            for (EquivalencePartition tv : targetParam.getPartitions()) {
                if (!rule.test(sv, tv)) {
                    logger.trace("Rule interaction found for rule on {} with {}: rule({}, {}) -> false",
                            sourceParam.getName(), targetParam.getName(), sv.getName(), tv.getName());
                    return true;
                }
                if (!rule.test(tv, sv)) {
                    logger.trace(
                            "Rule interaction found (symmetric check) for rule on {} with {}: rule({}, {}) -> false",
                            sourceParam.getName(), targetParam.getName(), tv.getName(), sv.getName());
                    return true;
                }
            }
        }
        logger.trace("No rule interaction found for rule on {} with {}", sourceParam.getName(), targetParam.getName());
        return false;
    }
}