 ...
 * <p>
 * This algorithm generates test combinations that cover all pairs of parameter
 * values while respecting compatibility rules. It uses a greedy approach to
 * build combinations that maximize coverage of value pairs.
 */
public class PairwiseAlgorithm extends GenerationAlgorithm {
  ...
  public CombinationTable generate(TestInput input, int nWiseOrLimit) {
    // nWiseOrLimit is not used by this PairwiseAlgorithm, but is part of the
    // interface.
    logger.debug("PairwiseAlgorithm.generate() called. Input parameters: {}. (nWiseOrLimit={} ignored)",
    ...
    logger.info("Generating pairwise combinations for {} parameters. Total pairs to cover: {}", parameters.size(),
        pairs.size());
    if (pairs.isEmpty() && parameters.size() >= 2) {
      ...
      logger.warn("Initial pair generation resulted in zero pairs to cover. Returning empty table.");
      return new CombinationTable(new ArrayList<>());
    }
    ...
    while (!pairs.isEmpty()) {
      ...
      logger.info("Starting new combination generation cycle. Pairs to cover: {} (Last cycle had: {})", pairs.size(),
          lastLoopPairsCount);
      ...
      if (combination != null && combination.getSetCount() > 0 && isValidCombination(combination)) {
        ...
        while (pairIterator.hasNext()) {
          ...
          if (isPairCovered(pair, combination)) {
            logger.trace("Pair covered and removed: {} ({}) + {} ({}) by {}", pair.param1.getName(),
                pair.value1.getName(), pair.param2.getName(), pair.value2.getName(), combination.toString());
            pairIterator.remove();
            pairsRemovedCount++;
          }
        }
        logger.info("Removed {} pairs covered by this combination (out of {}). Remaining pairs: {}", pairsRemovedCount,
            initialPairCountForCycle, pairs.size());
        ...
        if (pairsRemovedCount == 0 && !pairs.isEmpty()) {
          ...
          logger.warn(
              ...
              combination.toString(), pairs.size());
    ...
    logger.info("Pairwise generation finished. Generated {} combinations in total.", combinations.size());
  ...
  private void logRemainingPairs(List<ValuePair> pairs) {
    ...
    for (int i = 0; i < Math.min(pairs.size(), 10); i++) {
      ...
      logger.warn("  - Pair: Param1={}, Value1={}, Param2={}, Value2={}", p.param1.getName(), p.value1.getName(),
          p.param2.getName(), p.value2.getName());
    }
    if (pairs.size() > 10) {
  ...
  private Combination buildCombination(List<TestParameter> parameters, List<ValuePair> pairs) {
    ...
    logger.debug("Attempting to build combination for {} parameters with {} remaining pairs.", parameters.size(),
        pairs.size());
    ...
    while (!pairQueue.isEmpty()) {
      ...
      logger.debug("[Iter {}] Polled pair: {} ({}) + {} ({})", iteration, currentPair.param1.getName(),
          currentPair.value1.getName(), currentPair.param2.getName(), currentPair.value2.getName());
      ...
      if (tryAddPairToCombination(combination, currentPair)) {
        logger.debug("[Iter {}] Successfully ADDED pair directly. Combination now: {}", iteration,
      ...
      if (!tryAdjustCombination(combination, currentPair)) {
        ...
        // re-ordering.
        logger.debug("[Iter {}] Could NOT ADJUST combination for pair. Pair remains uncovered for now.", iteration);
      ...
      logger.debug("[Iter {}] Successfully ADJUSTED combination for pair. Combination now: {}", iteration,
          combination.toString());
      if (combination.isFilled() && isValidCombination(combination)) {
        logger.debug("[Iter {}] Combination is FILLED and VALID after adjusting for pair.", iteration);
      }
    }
    ...
    if (isFilled && isValid) {
      ...
      logger.warn("RETURN_LOGIC_BRANCH_3_NULL_CASE. State: {}, Filled: {}, Valid: {}, SetCount: {}",
          combination.toString(), isFilled, isValid, setCount);
      return null;
    }
  ...
  private boolean tryAddPairToCombination(Combination combination, ValuePair pair) {
    logger.debug("  -- tryAddPairToCombination START for pair: {} ({}) + {} ({}) --", pair.param1.getName(),
        pair.value1.getName(), pair.param2.getName(), pair.value2.getName());
    ...
    if (combination.getValue(index1) == null && combination.getValue(index2) == null) {
      logger.debug("     Slots for both params are empty. Trying to set: {} -> {}, {} -> {}", pair.param1.getName(),
          pair.value1.getName(), pair.param2.getName(), pair.value2.getName());
      combination.setValue(index1, pair.value1);
      combination.setValue(index2, pair.value2);
      if (isValidCombination(combination)) {
        logger.debug("     SUCCESS (both empty). Combination state AFTER: {}", combination.toString());
        logger.debug("  -- tryAddPairToCombination END (true) --");
        return true;
      }
      logger.debug("     FAILED (both empty, invalid). Reverting. Combination state AFTER: {}", combination.toString());
    ...
    if (combination.getValue(index1) == null && combination.getValue(index2) != null
      ...
      logger.debug("     Slot for param1 ({}) empty, param2 ({}) matches pair ({}). Trying to set: {} -> {}",
          pair.param1.getName(), pair.param2.getName(), pair.value2.getName(), pair.param1.getName(),
          pair.value1.getName());
      EquivalencePartition originalP1 = combination.getValue(index1); // null
      combination.setValue(index1, pair.value1);
      if (isValidCombination(combination)) {
        logger.debug("     SUCCESS (param1 empty, param2 matched). Combination state AFTER: {}",
            combination.toString());
        logger.debug("  -- tryAddPairToCombination END (true) --");
        return true;
      }
      logger.debug("     FAILED (param1 empty, param2 matched, invalid). Reverting. Combination state AFTER: {}",
          combination.toString());
    ...
    // Can we set param2?
    if (combination.getValue(index2) == null && combination.getValue(index1) != null
        && combination.getValue(index1).equals(pair.value1)) {
      logger.debug("     Slot for param2 ({}) empty, param1 ({}) matches pair ({}). Trying to set: {} -> {}",
          pair.param2.getName(), pair.param1.getName(), pair.value1.getName(), pair.param2.getName(),
          pair.value2.getName());
      EquivalencePartition originalP2 = combination.getValue(index2); // null
      combination.setValue(index2, pair.value2);
      if (isValidCombination(combination)) {
        logger.debug("     SUCCESS (param2 empty, param1 matched). Combination state AFTER: {}",
            combination.toString());
        logger.debug("  -- tryAddPairToCombination END (true) --");
        return true;
      }
      logger.debug("     FAILED (param2 empty, param1 matched, invalid). Reverting. Combination state AFTER: {}",
          combination.toString());
    ...
    if (combination.getValue(index1) != null && combination.getValue(index1).equals(pair.value1) &&
        combination.getValue(index2) != null && combination.getValue(index2).equals(pair.value2)) {
  ...
  private boolean tryAdjustCombination(Combination combination, ValuePair pair) {
    logger.debug("    -- tryAdjustCombination START for pair: {} ({}) + {} ({}) --", pair.param1.getName(),
        pair.value1.getName(), pair.param2.getName(), pair.value2.getName());
    ...
    if (originalValueP1 != null && !originalValueP1.equals(pair.value1)) {
      logger.debug("       Attempting to adjust P1: Current P1 ({}) is {}, pair needs {}.", pair.param1.getName(),
          originalValueP1.getName(), pair.value1.getName());
      ...
      if (currentValueP2 == null) { // If P2 is not set, set it to pair's P2
        ...
        combination.setValue(index2, pair.value2);
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P1 to {}, set P2 to {}. Valid. Combo: {}", pair.value1.getName(),
              pair.value2.getName(), combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        combination.setValue(index2, null); // Revert P2
        logger.debug("       P2 was null, set to {}, but combo invalid. Reverted P2.", pair.value2.getName());
        ...
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P1 to {}. P2 matched. Valid. Combo: {}", pair.value1.getName(),
              combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        ...
        logger.debug("       P2 is {} (mismatched pair value {}). Attempting to change P2 to {}.",
            currentValueP2.getName(), pair.value2.getName(), pair.value2.getName());
        EquivalencePartition originalConflictingP2 = currentValueP2; // Store it before changing
        combination.setValue(index2, pair.value2);
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P1 to {}, changed P2 from {} to {}. Valid. Combo: {}",
              pair.value1.getName(), originalConflictingP2.getName(), pair.value2.getName(), combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        combination.setValue(index2, originalConflictingP2); // Revert P2 to its pre-adjustment state
        logger.debug("       Changed P2 to {}, but combo invalid. Reverted P2 to {}.", pair.value2.getName(),
            originalConflictingP2.getName());
      }
      combination.setValue(index1, originalValueP1); // Revert P1 if all attempts above failed
      logger.debug("       All P1 adjustment attempts failed. Reverted P1 to {}. Combo: {}", originalValueP1.getName(),
          combination.toString());
    }
    ...
    if (originalValueP2 != null && !originalValueP2.equals(pair.value2)) {
      logger.debug("       Attempting to adjust P2: Current P2 ({}) is {}, pair needs {}.", pair.param2.getName(),
          originalValueP2.getName(), pair.value2.getName());
      ...
      if (currentValueP1 == null) { // If P1 is not set, set it to pair's P1
        ...
        combination.setValue(index1, pair.value1);
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P2 to {}, set P1 to {}. Valid. Combo: {}", pair.value2.getName(),
              pair.value1.getName(), combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        combination.setValue(index1, null); // Revert P1
        logger.debug("       P1 was null, set to {}, but combo invalid. Reverted P1.", pair.value1.getName());
        ...
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P2 to {}. P1 matched. Valid. Combo: {}", pair.value2.getName(),
              combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        ...
        logger.debug("       P1 is {} (mismatched pair value {}). Attempting to change P1 to {}.",
            currentValueP1.getName(), pair.value1.getName(), pair.value1.getName());
        EquivalencePartition originalConflictingP1 = currentValueP1; // Store it before changing
        combination.setValue(index1, pair.value1);
        if (isValidCombination(combination)) {
          logger.debug("       SUCCESS: Changed P2 to {}, changed P1 from {} to {}. Valid. Combo: {}",
              pair.value2.getName(), originalConflictingP1.getName(), pair.value1.getName(), combination.toString());
          logger.debug("    -- tryAdjustCombination END (true) --");
          return true;
        }
        combination.setValue(index1, originalConflictingP1); // Revert P1 to its pre-adjustment state
        logger.debug("       Changed P1 to {}, but combo invalid. Reverted P1 to {}.", pair.value1.getName(),
            originalConflictingP1.getName());
      }
      combination.setValue(index2, originalValueP2); // Revert P2 if all attempts above failed
      logger.debug("       All P2 adjustment attempts failed. Reverted P2 to {}. Combo: {}", originalValueP2.getName(),
          combination.toString());
    }
  ...
  private boolean isPairCovered(ValuePair pair, Combination combination) {
    ...
    return value1 != null && value2 != null &&
        value1.equals(pair.value1) && value2.equals(pair.value2);
  }
  ...
  private static class ValuePair {
    ...
    ValuePair(TestParameter param1, EquivalencePartition value1,
        TestParameter param2, EquivalencePartition value2) {
      this.param1 = param1;
      this.value1 = value1;
      this.param2 = param2;
      this.value2 = value2;
    ...
    public boolean equals(Object o) {
      ...
      return param1.equals(other.param1) && value1.equals(other.value1) &&
          param2.equals(other.param2) && value2.equals(other.value2);
    }
