...
/**
 * An equivalence partition that cycles through a set of equivalent values.
 * For example, when testing browser versions, you might want to cycle through
 * several versions of Chrome that are considered equivalent for testing purposes.
 * 
 * <p>The partition automatically cycles through all values, returning to the first
 * value after reaching the end. This is useful for ensuring test coverage across
 * multiple equivalent values while maintaining the pairwise testing efficiency.</p>
 * 
 * <p>Example usage:</p>
 * <pre>
...
public class CyclingPartition<T> extends BaseEquivalencePartition<T> {
    ...
    /**
     * Creates a new cycling partition with a name, default value, and a collection of equivalent values.
     *
     ...
     * The first value in the collection will be used as the default value.
     *
     * @param name The name of this partition (e.g., "Chrome")
     * @param equivalentValues Collection of values to cycle through
     */
     ...
     * This method is thread-safe and will automatically wrap around to the first value
    ...
    @Override
    public T getValue() {
        return equivalentValues.get(
            currentIndex.getAndUpdate(i -> (i + 1) % equivalentValues.size())
        );
    }
}
