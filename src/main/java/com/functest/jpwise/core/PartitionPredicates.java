...
/**
 * Utility class providing factory methods for creating common predicates to match
 * EquivalencePartition instances. This class is part of the modern DSL for defining conditions on
 * EquivalencePartition instances.
 *
 * <p>Example usage:
 *
 * <pre>
...
public final class PartitionPredicates {
  ...
  public static Predicate<EquivalencePartition<?>> parameterNameIs(String parameterName) {
    return partition ->
        partition.getParentParameter() != null
  ...
  /**
   * Creates a predicate that matches an EquivalencePartition by checking if its value is in a
   * collection.
   *
  ...
  /**
   * Creates a predicate that matches an EquivalencePartition by checking if its name is in a
   * collection.
   *
  ...
  /**
   * Creates a predicate that matches an EquivalencePartition by checking if its name starts with a
   * prefix.
   *
  ...
  /**
   * Creates a predicate that matches an EquivalencePartition by checking if its value contains a
   * substring.
   *
  ...
  public static Predicate<EquivalencePartition<?>> valueContains(String substring) {
    return partition ->
        partition.getValue() != null && partition.getValue().toString().contains(substring);
  }
   ...
   */
  @SafeVarargs
  public static Predicate<EquivalencePartition<?>> and(
      Predicate<EquivalencePartition<?>>... predicates) {
    return partition -> {
   ...
   */
  @SafeVarargs
  public static Predicate<EquivalencePartition<?>> or(
      Predicate<EquivalencePartition<?>>... predicates) {
    return partition -> {
      for (Predicate<EquivalencePartition<?>> predicate : predicates) {
        if (predicate.test(partition)) {
   ...
   */
  public static Predicate<EquivalencePartition<?>> not(
      Predicate<EquivalencePartition<?>> predicate) {
    return predicate.negate();
  }

  /**
   * Creates a predicate from a legacy PartitionMatcher. This is a bridge method to help transition
   * from matcher-based to predicate-based code.
   *
  ...
  /**
   * Creates a predicate that matches a field using a legacy Field enum and operator. This is a
   * bridge method to help transition from matcher-based to predicate-based code.
   *
  ...
  public static Predicate<EquivalencePartition<?>> matches(
      PartitionMatcher.Field field, ConditionOperator op, Object value) {
    return new PartitionMatcher(field, op, value);
  }

  /**
   * Creates a predicate that matches two fields using legacy Field enums and operators. This is a
   * bridge method to help transition from matcher-based to predicate-based code.
   *
