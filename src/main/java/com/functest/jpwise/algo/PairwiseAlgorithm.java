 ...
 * minimal set of test cases that covers all possible pairs of parameter values while respecting
 * compatibility rules.
 *
 * <p>The algorithm works in two main phases:
 *
 * <ol>
 *   <li>Generate all possible pairs of input values (equivalence partitions)
 *   <li>Build complete test cases by combining compatible partitions and getting their values
 * </ol>
 *
 * <p>The algorithm uses a tunable "jump" parameter that affects how pairs are selected from the
 * queue. A larger jump value may result in fewer test cases but takes longer to compute.
 *
 * <p>Example usage:
 *
 * <pre>
...
public class PairwiseAlgorithm extends GenerationAlgorithm {
  ...
  /**
   * Map tracking which parameter value pairs have been covered. The key is a string representation
   * of the pair, and the value is either COMPLETED or PENDING.
   */
   ...
   * fewer test cases but increases computation time.
   */
  private int _jump;
  ...
  /**
   * Generates a set of test cases that cover all possible pairs of parameter values. The algorithm
   * first generates all possible pairs, then builds complete test cases by combining compatible
   * pairs.
   *
  ...
  /**
   * Generates all possible pairs of parameter values that need to be covered. This is the first
   * phase of the algorithm where we identify all pairs that must appear in at least one test case.
   */
  ...
  /**
   * Generates all possible pairs between values of two parameters. Only compatible pairs are added
   * to the queue.
   *
  ...
  /**
   * Builds a complete test case by starting with a pair from the queue and adding compatible values
  ...
  /**
   * Completes a partial combination by adding compatible values (equivalence partitions) for all
   * parameters. This method tries to find values that are compatible with all values (partitions)
      ...
      if (combination.getValue(i) == null) {
        ...
        for (EquivalencePartition value : shuffledPartitions) {
          ...
          if (combination.checkNoConflicts(this)) {
            ...
            logger.trace(
                combination + " contains incompatible values (changed value was: " + value + ")?");
          }
        }
  ...
  /**
   * Marks all pairs in a combination as completed in the combination map. This helps track which
   * pairs have been covered by test cases.
   *
