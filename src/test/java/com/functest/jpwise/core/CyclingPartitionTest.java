...
public class CyclingPartitionTest {
    ...
    public void testBasicCycling() {
        CyclingPartition<String> partition = new CyclingPartition<>(
            "Chrome", "116.0", Arrays.asList("116.0", "116.1", "116.2")
        );
    ...
    public void testAutomaticReset() {
        CyclingPartition<Integer> partition = new CyclingPartition<>(
            "Resolution", 1080, Arrays.asList(1080, 1440, 2160)
        );
        ...
        assertEquals(partition.getValue(), Integer.valueOf(1080), "Should automatically reset to first value");
    }

    @Test
    public void testGetName() {
        CyclingPartition<String> partition = new CyclingPartition<>(
            "Browser", "Chrome", Arrays.asList("Chrome", "Firefox")
        );
    ...
    public void testCompatibility() {
        CyclingPartition<String> safari = new CyclingPartition<>(
            "Safari", "17.0", Arrays.asList("17.0", "17.1")
        );
        CyclingPartition<String> macOS = new CyclingPartition<>(
            "macOS", "14.1", Arrays.asList("14.1", "14.2")
        );
        ...
        TestParameter browserParam = new TestParameter("browser", Arrays.<EquivalencePartition<?>>asList(safari),
            Arrays.asList((v1, v2) -> v1.getName().equals("Safari") && v2.getName().equals("macOS")));
