...
public class PairwiseAlgorithmTest {
  ...
  public void testLargeNoRulesOptimalPrimaryPass() {
    ...
    // Test with rule preprocessing OFF (new default)
    TestGenerator generatorWithPreproc =
        new TestGenerator(input); // Uses new default: preprocessing OFF
    CombinationTable resultWithPreproc = generatorWithPreproc.generate(new PairwiseAlgorithm());
    logger.info(
        "Large no-rules (with preproc OFF by default): Generated {} combinations. Expected {}-{}. All pairs: {}",
        resultWithPreproc.size(),
        25, // LOWER_BOUND_LARGE_NO_RULES (assuming this was the intended constant)
        70, // UPPER_BOUND_LARGE_NO_RULES (assuming this was the intended constant)
    ...
    assertTrue(
        ...
        "Should generate 25-70 combinations for 10x5 no-rules input (with preproc OFF by default). Actual: "
            + resultWithPreproc.size());
    assertAllPairsCovered(
  ...
  public void testAlgorithmHandlesOneSidedRuleWhenRuleOnSecondParameter() {
    ...
    for (Combination combo : res.combinations()) {
      ...
      String osVal = combo.getValue(osIdx).getName();
      assertFalse(
          "Windows".equals(osVal) && "Safari".equals(browserVal),
  ...
  public void testRuleWithThreeValuesAndOneInvalidPair_NoPreprocessing() {
    ...
    for (Combination combo : result.combinations()) {
      ...
      assertFalse(
          "Mobile".equals(deviceVal) && "4K".equals(resolutionVal),
